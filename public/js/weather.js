...
const search = function(){
    ...
    const request = async() =>  {
        ...
        document.getElementById("feelslike").innerHTML = "Feels like " + json.current.feelslike_c + "&deg C " + "/ "+ json.current.feelslike_f + "&deg F"
        document.getElementById("humidity").innerHTML = "Humidity is " + json.current.humidity
        document.getElementById("winds").innerHTML = "Winds blowing at " + json.current.wind_kph +" Kmph."
        document.getElementById("wimg").src = json.current.condition.icon 
    }
    request()
}